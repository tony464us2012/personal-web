    ...
    const CustomLink = ({ href, title, className='' }) => {
        ...
        return (
            <Link href={href} className={`${className} relative group`}>
                ...
                absolute -bottom-0.5 left-0 group-hover:w-full transition-[width] ease duration-300 mt-5 ${pathname === href ? 'w-full' : 'w-0'}`} >
                &nbsp;
                </span>
            </Link>
        )
...
const CustomMobileLink = ({ href, title, className='', toggle }) => {
  ...
  return (
    <header className= 'w-full px-32 py-8 font-medium flex items-center justify-between dark:text-light dark:bg-dark bg-light text-dark relative z-10 lg:px-16 md:px-12 sm:px-8'>
        
        <button className='flex-col justify-center items-center lg:flex lg-min:hidden' onClick={handleClick}>
        ...
        <div className='w-full flex justify-between items-center lg:hidden'>
            ...
            <nav className='flex items-center mr-auto flex-wrap'>
                
                <motion.a href='https://github.com/tony464us2012' target={"_blank"}
                whileHover={{y: -2}}
                whileTap={{scale: 0.9}}
                className='w-6 mr-3'
                ><GithubIcon /></motion.a>
